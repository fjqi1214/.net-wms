namespace BenQGuru.eMES.Web.UserControl
{
	...
	public class PageNavigator : System.Web.UI.UserControl
	{
		...
		public void AddPageNavigator( string text, string navigatorUrl )
		{
			...
			link.Text = text;
            if (navigatorUrl.Trim() != string.Empty && text != "主页" && text.ToUpper() != "STARTPAGE" &&
                navigatorUrl.Trim().ToUpper().EndsWith("FRptStartPage.aspx".ToUpper()) == false &&
                navigatorUrl.Trim().ToUpper().EndsWith("FReportViewMain.aspx".ToUpper()) == false)
			{
