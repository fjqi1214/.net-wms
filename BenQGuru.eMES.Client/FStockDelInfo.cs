...
namespace BenQGuru.eMES.Client
{
	...
	/// FStockRemove 的摘要说明。
	/// </summary>
	public class FStockDelInfo : System.Windows.Forms.Form
	{
		...
		private FStockOutput frmStockOut;
		/// <summary>
		/// 必需的设计器变量。
		/// </summary>
		private System.ComponentModel.Container components = null;

		public FStockDelInfo()
		{
			//
			// Windows 窗体设计器支持所必需的
			//
		...
		public FStockDelInfo(Form frm)
		{
			...
			if(frm is FStockOutput)
			{
				frmStockOut = (FStockOutput)frm;
			}
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(FStockDelInfo));
		...
		private void ucBtnOK_Click(object sender, System.EventArgs e)
		{
			...
			{
				this.ShowMessage("$CS_PLEASE_INPUT_DEL_INFO");	//请输入待移除的二维条码
			}
			else
			{
				...
				if(frmStockIn != null)
				{
					frmStockIn.DeleteInfor = this.rtxtInput.Text.ToString().ToUpper().Trim();
				}
				if(frmStockOut != null)
				{
					frmStockOut.DeleteInfor = this.rtxtInput.Text.ToString().ToUpper().Trim();
				}
				this.Close();
			}
		}
