...
{
    public partial class FLotCollectionCartonForLot : BaseForm
    {
        ...
        private void SetInputMessageByFlowControl()
        {
            ...
            {
                ucMessage.AddEx(this.m_FunctionName, "", new UserControl.Message(">>$CS_Please_Input_CartonNo_Again"), false);
                this.ucLabelCartonNoForLot.TextFocus(false, true);
            }
        }
        ...
        private void ucLabelCartonNoForLot_TxtboxKeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == '\r')
            {
                if (CartonPack())
                {
                    ...
                    {
                        ucMessage.AddEx(this.m_FunctionName, "$CS_LotNo: " + lotCode.Trim().ToUpper(), new UserControl.Message(MessageType.Success, "$CS_LOT_HAS_PACKED"), false);
                        this.m_FlowControl = 1;
                        this.SetInputMessageByFlowControl();
                    }
                    else
        ...
        private bool CheckCarton(string cartonNo)
        {
            ...
            if (chkCartonLen.Checked && chkCartonLen.Value.Trim().Length > 0)
            {
                ...
                {
                    ucMessage.AddEx(this.m_FunctionName, "$CS_CARTON_NO: " + this.ucLabelCartonNoForLot.Value, new UserControl.Message(MessageType.Error, "$CS_CARTON_NO_LEN_CHECK_FAIL"), false);
                    return false;
                }
            }
            #endregion
            ...
            if (chkCartonFChar.Checked && chkCartonFChar.Value.Trim().Length > 0)
            {
                ...
                {
                    ucMessage.AddEx(this.m_FunctionName, "$CS_CARTON_NO: " + this.ucLabelCartonNoForLot.Value, new UserControl.Message(MessageType.Error, "$CS_CARTON_NO_FCHAR_CHECK_FAIL"), false);
                    return false;
                }
            }
            ...
            if (objCartonInfo != null)
            {
                ...
                {
                    ucMessage.AddEx(this.m_FunctionName, "$CS_CARTON_NO: " + this.ucLabelCartonNoForLot.Value, new UserControl.Message(MessageType.Error, "$CARTON_ALREADY_FILL_OUT"), false);
                    return false;
                }
            }
            #endregion
                ...
                if (cartonNo != string.Empty)
                {
                    ...
                    else if (cartonNo != String.Empty)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                messages.Add(new UserControl.Message(MessageType.Error, "$CS_PLEASE_MAINTEIN_ITEMCARTON $CS_Param_Lot =" + lotCode));
                            }
                            else
                            {
                                if (carton.CAPACITY == carton.COLLECTED)
        ...
        private void InitMessage(string cartonNo, bool result)
        {
            if (result)
            {
                this.ucMessage.AddEx(this.m_FunctionName, "", new UserControl.Message(MessageType.Success, cartonNo + " $CS_Add_Success"), false);
                this.ucLabelCartonNoForLot.Value = "";
                ApplicationRun.GetQtyForm().RefreshQty();
            }
            else
            {
                this.ucMessage.AddEx(this.m_FunctionName, "$CS_CARTON_NO: " + cartonNo, new UserControl.Message(MessageType.Error, cartonNo + " $CS_Add_Fail"), true);
            }
        ...
        private void ultraGridDetail_InitializeLayout(object sender, Infragistics.Win.UltraWinGrid.InitializeLayoutEventArgs e)
        {
            ...
            e.Layout.Bands[0].Columns["CartonCode"].Width = 200;
            e.Layout.Bands[0].Columns["CartonQty"].Width = 200;
