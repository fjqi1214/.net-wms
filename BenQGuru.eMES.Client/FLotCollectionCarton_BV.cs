...
{
    public partial class FLotCollectionCarton_BV : BaseForm
    {
        private int m_FlowControl = 1;
        private int m_CartonCapacity = 0;
        private bool m_CartonIsFull = false;
        private string m_FunctionName = "FLotCollectionCarton_BV";
        ...
        public FLotCollectionCarton_BV()
        {
            InitializeComponent();
        ...
        private void FLotCollectionCarton_BV_Load(object sender, EventArgs e)
        {
            ...
            //this.InitGridLanguage(this.ultraGridDetail);
        }
        ...
        private void SetInputMessageByFlowControl()
        {
            ...
            {
                ucMessage.AddEx(this.m_FunctionName, "", new UserControl.Message(">>$CS_Please_Input_CartonNo"), false);
                this.ucLabelCartonNo.TextFocus(false, true);
            }
            else if (m_FlowControl == 2)   //请输入产品批次条码
            {
                ucMessage.AddEx(this.m_FunctionName, "", new UserControl.Message(">>$CS_Please_Input_LotCode"), false);
                this.ucLabelLotCodeForCarton.TextFocus(false, true);
            }
            else if (m_FlowControl == 3)   //请再输入产品批次条码
            {
                ucMessage.AddEx(this.m_FunctionName, "", new UserControl.Message(">>$CS_Please_Input_LotCode_Again"), false);
                this.ucLabelLotCodeForCarton.TextFocus(false, true);
            }
        }
        ...
        private bool CheckCarton(string cartonNo)
        {
            ...
            if (chkCartonLen.Checked && chkCartonLen.Value.Trim().Length > 0)
            {
                ...
                {
                    ucMessage.AddEx(this.m_FunctionName, "$CS_CARTON_NO: " + this.ucLabelCartonNo.Value, new UserControl.Message(MessageType.Error, "$CS_CARTON_NO_LEN_CHECK_FAIL"), false);
                    this.m_FlowControl = 1;
                    this.SetInputMessageByFlowControl();
                    return false;
                }
            ...
            if (chkCartonFChar.Checked && chkCartonFChar.Value.Trim().Length > 0)
            {
                ...
                {
                    ucMessage.AddEx(this.m_FunctionName, "$CS_CARTON_NO: " + this.ucLabelCartonNo.Value, new UserControl.Message(MessageType.Error, "$CS_CARTON_NO_FCHAR_CHECK_FAIL"), false);
                    this.m_FlowControl = 1;
                    this.SetInputMessageByFlowControl();
                    return false;
                }
            ...
            if (objCartonInfo != null)
            {
                ...
                {
                    ucMessage.AddEx(this.m_FunctionName, "$CS_CARTON_NO: " + this.ucLabelCartonNo.Value, new UserControl.Message(MessageType.Error, "$CARTON_ALREADY_FILL_OUT"), false);
                    this.m_FlowControl = 1;
                    this.SetInputMessageByFlowControl();
                    return false;
                }
        ...
        private bool CartonPack()
        {
            ...
            if (chkLotCodeLen.Checked && chkLotCodeLen.Value.Trim().Length > 0)
            {
                ...
                {
                    ucMessage.AddEx(this.m_FunctionName, "$CS_LotNo: " + this.ucLabelLotCodeForCarton.Value, new UserControl.Message(MessageType.Error, "$CS_LOT_NO_LEN_CHECK_FAIL"), false);
            ...
            if (chkLotCodeFChar.Checked && chkLotCodeFChar.Value.Trim().Length > 0)
            {
                ...
                {
                    ucMessage.AddEx(this.m_FunctionName, "$CS_LotNo: " + this.ucLabelLotCodeForCarton.Value, new UserControl.Message(MessageType.Error, "$CS_LOT_NO_FCHAR_CHECK_FAIL"), false);
                    this.m_FlowControl = 3;
                    this.SetInputMessageByFlowControl();
                    return false;
                }
            ...
            {
                ucMessage.AddEx(this.m_FunctionName, "$CS_LotNo: " + lotCode.Trim().ToUpper(), new UserControl.Message(MessageType.Error, "$CS_LOT_HAS_PACKED"), false);
                this.m_FlowControl = 3;
                this.SetInputMessageByFlowControl();
                return false;
            }
            ...
            if (objects != null)
            {
                ...
                {
                    ucMessage.AddEx(this.m_FunctionName, "", new UserControl.Message(MessageType.Error, "$OneCarton_OneMoCode"), true);
                    this.m_FlowControl = 3;
                    this.SetInputMessageByFlowControl();
                    return false;
                }
                ...
                if (obj != null)
                {
                    ...
                    {
                        ucMessage.AddEx(this.m_FunctionName, "", new UserControl.Message(MessageType.Error, "$OneCarton_OneProduct"), true);
                        this.m_FlowControl = 3;
                        this.SetInputMessageByFlowControl();
                        return false;
                    }
        ...
        {            
            ...
            {
                ucMessage.AddEx(this.m_FunctionName, "$CS_LotNo: " + lotCode, new UserControl.Message(MessageType.Error, "$LotNoProductInfo"), true);
                return false;
            }
            ...
            {
                ucMessage.AddEx(this.m_FunctionName, "$CS_LotNo: " + lotCode, new UserControl.Message(MessageType.Error, "$LotIsUnfinished"), true);
                return false;
            }
            if (!objSimulation.LotStatus.Equals(LotStatusForMO2LotLink.LOTSTATUS_USE))//使用的批才可包装
            {
                ucMessage.AddEx(this.m_FunctionName, "$CS_LotNo: " + lotCode, new UserControl.Message(MessageType.Error, "$LotIsInvalid"), true);
                return false;
            }
            ...
            if (item == null)
            {
                ucMessage.AddEx(this.m_FunctionName, "$CS_LotNo: " + lotCode, new UserControl.Message(MessageType.Error, "$Error_ItemCode_NotExist $Domain_Item=" + objSimulation.ItemCode), true);
                return false;
            }
        ...
        private void DoAction(string lotCode)
        {
            ...
            try
            {
                messages.AddMessages(AddSingleIDIntoCartonAndTry(lotCode));
        ...
        private void InitMessage(string lotCode, bool result)
        {
            if (result)
            {
                this.ucMessage.AddEx(this.m_FunctionName, "", new UserControl.Message(MessageType.Success, lotCode + " $CS_Add_Success"), false);
                this.ucLabelLotCodeForCarton.Value = "";
                ApplicationRun.GetQtyForm().RefreshQty();
            }
            else
            {
                this.ucMessage.AddEx(this.m_FunctionName, "$CS_LotNo: " + lotCode, new UserControl.Message(MessageType.Error, lotCode + " $CS_Add_Fail"), true);
            }
        ...
        private void ultraGridDetail_InitializeLayout(object sender, Infragistics.Win.UltraWinGrid.InitializeLayoutEventArgs e)
        {
            ...
            e.Layout.Bands[0].Columns["lotCode"].Width = 200;
            e.Layout.Bands[0].Columns["lotQty"].Width = 200;

            InitGridLanguage(ultraGridDetail);
        }
