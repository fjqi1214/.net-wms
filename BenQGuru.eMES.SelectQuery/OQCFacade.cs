...
namespace BenQGuru.eMES.OQC
{
    ...
    public class OQCFacade : MarshalByRefObject
    {
        ...
        string dqmCode, string cusMCode, string gfhwItemCode, int inclusive, int exclusive)
        {
            return QueryOQC2(userCodes, string.Empty, pickNo, oqcNo, status, bAppDate, eAppDate, cartonno, sn, invno, dqmCode, cusMCode, gfhwItemCode, inclusive, exclusive);
      ...
      string dqmCode, string cusMCode, string gfhwItemCode)
        {
            return QueryOQC2Count(userCodes, string.Empty, pickNo, oqcNo, status, bAppDate, eAppDate, cartonno, sn, invno, dqmCode, cusMCode, gfhwItemCode);
         ...
         string dqmCode, string cusMCode, string gfhwItemCode, int inclusive, int exclusive)
        {
            ...
            sql = @"SELECT A.OQCNO,A.OQCTYPE,A.PICKNO,A.CARINVNO,A.STATUS,A.APPDATE,APPTIME,A.CUSER,B.PICKTYPE,B.INVNO,B.GFFLAG,C.APPQTY,C.NGQTY,C.RETURNQTY FROM TBLOQC A inner join TBLPICK B ON A.PICKNO=B.PICKNO LEFT JOIN
...
A.PICKNO IN (
 ...
 SELECT PICKNO FROM TBLPICK WHERE INSTORAGECODE IN(SELECT PARAMCODE FROM TBLSYSPARAM WHERE PARAMGROUPCODE in (" + SqlFormat(userCodes) + @"))) T 
            ...
            if (!string.IsNullOrEmpty(cartonno))
            {
                ...
                sql += @" AND A.OQC in (select distinct OQCNO from TBLOQCDETAIL where cartonno='" + cartonno + "' )";
            }
...
WHERE 1=1 ", DomainObjectUtility.GetDomainObjectFieldsString(typeof(OQCDetailEC)));
        ...
        {
            string sql = @"SELECT B.CLOSECOUNT,A.total FROM (SELECT count(*) total,pickno FROM TBLOQC WHERE PICKNO='" + pickno + @"' group by pickno) A LEFT JOIN
