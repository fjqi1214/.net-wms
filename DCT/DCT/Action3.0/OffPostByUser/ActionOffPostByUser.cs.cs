...
namespace BenQGuru.eMES.Common.DCT.Action
{
    ...
    /// </summary>
    public class ActionCollectOffPost : BaseDCTAction
    {

        List<string> _userList = new List<string>();

        public ActionCollectOffPost()
        {
        ...
        public override Messages Action(object act)
        {
            ...
            if (act.ToString().ToUpper() == "CONFIRM")
            {
                ...
                if (msgs.IsSuccess())
                {
                    msgs = performanceFacade.GoOffPost(_userList, maintainUserCode);

                    if (msgs.IsSuccess())
                    {
                        msgs.ClearMessages();
                        msgs.Add(new UserControl.Message(MessageType.Success, "$Message_DCTGoOffPostSuccessfully [" + _userList.Count.ToString() + "] $Message_People"));
                    }
                }
            ...
            if (userIsExist)
            {
                ...
                msgs.Add(new UserControl.Message(MessageType.Normal, "$Message_Confirm_GoOffPost"));
                ProcessBeforeReturn(this.Status, msgs);
                return msgs;
            }
            ...
            msgs = performanceFacade.CheckBeforeGoOffPost(_userList);
