...
namespace BenQGuru.eMES.Web
{
    ...
    public class MenuBuilder
    {
        ...
        public Item GetReportViewMenuRoot(string reportViewMenuCode, ControlLibrary.Web.Language.LanguageComponent languageComponent, IDomainDataProvider domainDataProvider)
        {
        ...
        private void AddMenuItemFromRptViewEntryList(Item rootItem, object[] entryArray, string parentCode)
        {
            foreach (RptViewEntry entry in entryArray)
            {
                if (entry.Visible == FormatHelper.TRUE_STRING && string.Compare(entry.ParentEntryCode, parentCode, true) == 0)
                {
                    ...
                    if (entry.EntryType == ReportEntryType.Report)
                        item.TargetUrl = "ReportView/FRptViewMP.aspx?reportid=" + entry.ReportID.ToString();
        ...
        private Infragistics.WebUI.UltraWebNavigator.Item BuildUltraMenuItem(MenuTreeNode node, ControlLibrary.Web.Language.LanguageComponent languageComponent, string reportViewMenuCode, Item reportViewMenuItem)
        {
            Infragistics.WebUI.UltraWebNavigator.Item item = new Infragistics.WebUI.UltraWebNavigator.Item();
            ...
            {
                foreach (Item reportViewitem in reportViewMenuItem.Items)
                {
