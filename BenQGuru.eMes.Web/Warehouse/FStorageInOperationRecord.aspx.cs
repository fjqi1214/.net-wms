...
{
    public partial class FStorageInOperationRecord : BaseMPageNew
    {
        ...
        protected override void InitWebGrid()
        {
            ...
            this.gridHelper.AddColumn("ReceiveDate", "到货初检日期", null);
            this.gridHelper.AddColumn("ReceiveUSER", "到货初检人", null);
            this.gridHelper.AddColumn("IQCDate", "IQC日期", null);
            this.gridHelper.AddColumn("IQCUser", "IQC检验人", null);
            this.gridHelper.AddColumn("SQEDate", "SQE判定日期", null);
            this.gridHelper.AddColumn("SQEUser", "SQE操作人", null);
            this.gridHelper.AddColumn("INSTORAGEDate", "上架日期", null);
            this.gridHelper.AddColumn("INSTORAGEUser", "上架人", null);
            //多语言
        ...
        protected override DataRow GetGridRow(object obj)
        {
            ...
            StorageInOperationRecord s = (StorageInOperationRecord)obj;
            row["STORAGECODE"] = s.STORAGECODE;
            row["STNO"] = s.STNO;
            row["INVNO"] = s.INVNO;
            row["ReceiveDate"] = FormatHelper.ToDateString(s.ReceiveDate);
            row["ReceiveUSER"] = string.Join(",", s.ReceiveOpers.ToArray());
            row["IQCDate"] = FormatHelper.ToDateString(s.IQCDate);
            row["IQCUser"] = string.Join(",", s.IQCOpers.ToArray());
            row["SQEDate"] = FormatHelper.ToDateString(s.IQCSQEDate);
            row["SQEUser"] = string.Join(",", s.IQCSQEOpers.ToArray());
            row["INSTORAGEDate"] = FormatHelper.ToDateString(s.InStorageDate);
            row["INSTORAGEUser"] = string.Join(",", s.InStorageOpers.ToArray());
            return row;
        }
        ...
        protected override object[] LoadDataSource(int inclusive, int exclusive)
        {
            ...
            return this.facade.QueryASNs(this.txtStorageCodeEdit.Text,
                                         this.txtStorageInASNQuery.Text, this.txtInvNoQuery.Text,
        ...
        protected override void cmdExport_Click(object sender, EventArgs e)
        {
            ...
            xls.CreateSheet("入库操作记录");
            xls.NewRow(0);
            xls.Cell(0, "库位", style);
            xls.Cell(1, "入库指令号", style);
            xls.Cell(2, "SAP单据号", style);
            xls.Cell(3, "到货初检人", style);
            xls.Cell(4, "到货初检日期", style);
            xls.Cell(5, "IQC操作人", style);
            xls.Cell(6, "IQC日期", style);
            xls.Cell(7, "SQE操作人", style);
            xls.Cell(8, "SQE判定日期", style);
            xls.Cell(9, "上架人", style);
            xls.Cell(10, "上架日期", style);
            int rowNum = 1;
            ...
            for (int i = 0; i < this.gridWebGrid.Rows.Count; i++)
            {
                ...
                string stno = this.gridWebGrid.Rows[i].Items.FindItemByKey("STNO").Text;
                xls.Cell(1, stno, style);
                string invno = this.gridWebGrid.Rows[i].Items.FindItemByKey("INVNO").Text;
                xls.Cell(2, invno, style);
                string receiveUser = this.gridWebGrid.Rows[i].Items.FindItemByKey("ReceiveUSER").Text;
                xls.Cell(3, receiveUser, style);
                string receiveDate = this.gridWebGrid.Rows[i].Items.FindItemByKey("ReceiveDate").Text;
                xls.Cell(4, receiveDate, style);
                string iqcUser = this.gridWebGrid.Rows[i].Items.FindItemByKey("IQCUser").Text;
                xls.Cell(5, iqcUser, style);
                string iqcDate = this.gridWebGrid.Rows[i].Items.FindItemByKey("IQCDate").Text;
                xls.Cell(6, iqcDate, style);
                string sqeUser = this.gridWebGrid.Rows[i].Items.FindItemByKey("SQEUser").Text;
                xls.Cell(7, sqeUser, style);
                string sqeDate = this.gridWebGrid.Rows[i].Items.FindItemByKey("SQEDate").Text;
                xls.Cell(8, sqeDate, style);
                string inUser = this.gridWebGrid.Rows[i].Items.FindItemByKey("INSTORAGEUser").Text;
                xls.Cell(9, inUser, style);
                string inDate = this.gridWebGrid.Rows[i].Items.FindItemByKey("INSTORAGEDate").Text;
                xls.Cell(10, inDate, style);
                rowNum++;
