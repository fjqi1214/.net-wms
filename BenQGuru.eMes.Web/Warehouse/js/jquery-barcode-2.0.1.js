...
var jc = jQuery.noConflict(true);
  ...
  code128: {
    ...
    getDigit: function(code){
      ...
      while( i < code.length ){
        ...
        if ( tableCActivated ) {
         value = jc.barcode.intval(code.charAt(i) + code.charAt(i+1)); // Add two characters (numeric)    
		i += 2;
        } else {
  ...
  digitToCss: function($container, settings, digit, hri){
    var w = jc.barcode.intval(settings.barWidth);
    var h = jc.barcode.intval(settings.barHeight);
...
jc.fn.extend({
  barcode: function(datas, type, settings) {
    ...
    for(var name in jc.barcode.settings){
      if (settings[name] == undefined) settings[name] = jc.barcode.settings[name];
    }
    
    switch(type){
      case "std25":
      case "int25":
        digit = jc.barcode.i25.getDigit(code, crc, type);
        hri = jc.barcode.i25.compute(code, crc, type);
      break;
      case "ean8":
      case "ean13":
        digit = jc.barcode.ean.getDigit(code, type);
        hri = jc.barcode.ean.compute(code, type);
      break;
      case "code11":
        digit = jc.barcode.code11.getDigit(code);
        hri = code;
      break;
      case "code39":
        digit = jc.barcode.code39.getDigit(code);
        hri = code;
      break;
      case "code93":
        digit = jc.barcode.code93.getDigit(code, crc);
        hri = code;
      break;
      case "code128":
        digit = jc.barcode.code128.getDigit(code);
        hri = code;
      break
      case "codabar":
        digit = jc.barcode.codabar.getDigit(code);
        hri = code;
      break;
      case "msi":
        digit = jc.barcode.msi.getDigit(code, crc);
        hri = jc.barcode.msi.compute(code, crc);
      break;
      case "datamatrix":   
        digit = jc.barcode.datamatrix.getDigit(code, rect);
        hri = code;
        b2d = true;
      break; 
    }
    ...
    if (typeof(jc.barcode[fname]) == 'function') {
      jc.barcode[fname]($this, settings, digit, hri);
    }
