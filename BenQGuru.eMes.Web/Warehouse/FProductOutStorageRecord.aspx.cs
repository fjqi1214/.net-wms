...
{
    public partial class FProductOutStorageRecord : BaseMPageNew
    {
        ...
        protected override void InitWebGrid()
        {
            ...
            this.gridHelper.AddColumn("PICKNO", "拣货任务令", null);
            this.gridHelper.AddColumn("INVNO", "SAP单据号", null);
            this.gridHelper.AddColumn("CARINVNO", "箱单号", null);
            this.gridHelper.AddColumn("PICKTYPE", "出库类型", null);
            this.gridHelper.AddColumn("DeliveryDate", "出库日期", null);
            ...
            this.gridHelper.AddColumn("PROJECTNAME", "项目名称", null);
            this.gridHelper.AddColumn("ORDERNOkk", "合同号", null);
            this.gridHelper.AddColumn("CUSORDERNO", "客户订单号", null);
            this.gridHelper.AddColumn("ReceiverAddr", "收货地址", null);
            this.gridHelper.AddColumn("SHIPTOPARTY", "客户代码", null);
        ...
        protected override DataRow GetGridRow(object obj)
        {
            ...
            ProductOutstorageRecord s = (ProductOutstorageRecord)obj;

            row["PICKNO"] = s.PICKNO;
            row["INVNO"] = s.INVNO;
            row["CARINVNO"] = s.CARINVNO;
            row["PICKTYPE"] = s.PICKTYPE;
            row["DeliveryDate"] = FormatHelper.ToDateString(s.DeliveryDate);
            ...
            row["PROJECTNAME"] = s.ProjectName;
            row["ORDERNOkk"] = s.ORDERNO;
            row["CUSORDERNO"] = s.CUSORDERNO;



            row["ReceiverAddr"] = s.ReceiverAddr;
            row["SHIPTOPARTY"] = s.SHIPTOPARTY;
            return row;
        ...
        protected override object[] LoadDataSource(int inclusive, int exclusive)
        {
            ...
            return this.facade.QueryProductOutstorages(
                                         FormatHelper.TODateInt(dateInDateFromQuery.Text),
                                         FormatHelper.TODateInt(dateInDateToQuery.Text),txtDQMCodeQuery.Text,
                                         txtDQMDESCQuery.Text,txtSNQuery.Text ,
                                         txtSupplierLotNoQuery.Text,
                                         txtProjectNameQuery.Text,
                                         txtORDERNOQuery.Text,
                                         txtCustOrderQuery.Text,
                                         inclusive,
                                         exclusive
        ...
        protected override int GetRowCount()
        {
            ...
            return this.facade.QueryProductOutstoragesCount(FormatHelper.TODateInt(dateInDateFromQuery.Text),
                                         FormatHelper.TODateInt(dateInDateToQuery.Text), txtDQMCodeQuery.Text,
                                         txtDQMDESCQuery.Text, txtSNQuery.Text,
                                         txtSupplierLotNoQuery.Text,
                                         txtProjectNameQuery.Text,
        ...
        protected override void cmdExport_Click(object sender, EventArgs e)
        {
            ...
            xls.CreateSheet("产品出库记录表");
            xls.NewRow(0);
            xls.Cell(0, "拣货任务令号", style);
            xls.Cell(1, "SAP单据号", style);
            xls.Cell(2, "箱单号", style);
            xls.Cell(3, "出库类型", style);
            xls.Cell(4, "出库日期", style);
            xls.Cell(5, "鼎桥物料编码", style);
            xls.Cell(6, "鼎桥物料编码描述", style);
            xls.Cell(7, "数量", style);
            xls.Cell(8, "SN", style);
       
            xls.Cell(9, "供应商代码", style);
            xls.Cell(10, "供应商批次号", style);
            xls.Cell(11, "项目名称", style);
            xls.Cell(12, "合同号", style);
            xls.Cell(13, "客户订单号", style);

            xls.Cell(14, "收货地址", style);
            xls.Cell(15, "客户代码", style);
            ...
            ProductOutstorageRecord[] records = this.facade.QueryProductOutstorages(
                                          FormatHelper.TODateInt(dateInDateFromQuery.Text),
            ...
            foreach (ProductOutstorageRecord record in records)
            {
                ...
                string CARINVNO = record.CARINVNO;
                xls.Cell(2, CARINVNO, style);

                string PICKTYPE = record.PICKTYPE;
                xls.Cell(3, PICKTYPE, style);

                string DeliveryDate = FormatHelper.ToDateString(record.DeliveryDate);
                xls.Cell(4, DeliveryDate, style);

                string DQMCODE = record.DQMCODE;
                xls.Cell(5, DQMCODE, style);


                string MDESC = record.MDESC;
                xls.Cell(6, MDESC, style);



                decimal PQTY = record.PQTY;
                xls.Cell(7, (int)PQTY, style);
                ...
                xls.Cell(9, VENDORCODE, style);

                string Supplier_LotNo = record.Supplier_LotNo;
                xls.Cell(10, Supplier_LotNo, style);



                string ProjectName = record.ProjectName;
                xls.Cell(11, ProjectName, style);


                string ORDERNO = record.ORDERNO;
                xls.Cell(12, ORDERNO, style);

                string CUSORDERNO = record.CUSORDERNO;
                xls.Cell(13, CUSORDERNO, style);
                string ReceiverAddr = record.ReceiverAddr;
                xls.Cell(14, ReceiverAddr, style);
                string SHIPTOPARTY = record.SHIPTOPARTY;
                xls.Cell(15, SHIPTOPARTY, style);
