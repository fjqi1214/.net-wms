...
{
	/// <summary>
	/// FReportCenterWeekPercent 的摘要说明。
	/// </summary>
	public partial class FReportCenterWeekPercent : BaseRQPage
	{
		...
		{
			return new FacadeFactory(base.DataProvider).CreateQueryFacade1().QueryRPTCenterWeekPercent(today,opCode);
		}
		...
		private void _processOWC(object[] dataSource)
		{
			...
			if( dataSource != null )
			{
				...
				{
					categories[i] = (dataSource[i] as RPTCenterWeekYield).ShiftDay.ToString();
					values[i] = (dataSource[i] as RPTCenterWeekYield).DayPercent;
				}
