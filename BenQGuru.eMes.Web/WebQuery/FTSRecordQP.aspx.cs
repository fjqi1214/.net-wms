...
namespace BenQGuru.eMES.Web.WebQuery
{
	...
	/// FTSRecordQP 的摘要说明。
	/// </summary>
	public partial class FTSRecordQP : BaseQPage
	{
		...
		protected System.Web.UI.WebControls.Label lblOperationQuery;
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if( !this.IsPostBack )
			{
                ...
                tsstatus.Items.Remove(TSStatus.TSStatus_RepeatNG);      //不包括重复测试
                // Added By Hi1/Venus.Feng on 20081013 for Hisense Version : not need Split
                tsstatus.Items.Remove(TSStatus.TSStatus_Split);         //不包含拆解
                // End Added
		...
		{
			this._gridHelper.AddColumn("Ts_SN",				"序列号",null);
			this._gridHelper.AddColumn("Ts_SNSeq",				"序列号顺序",null);
			this._gridHelper.AddColumn("Ts_Count",				"第N次不良",null);
			this._gridHelper.AddColumn("Ts_TsState",			"维修状态",null);
			this._gridHelper.AddLinkColumn("Ts_Details",	"维修信息",null);
			this._gridHelper.AddLinkColumn("Ts_ChangedItemDetails",	"换料信息",null);
			this._gridHelper.AddLinkColumn("Ts_ChangedItemDetailsSMT",	"SMT换料",null);
			this._gridHelper.AddColumn("Ts_ModelCode",			"产品别",null);
			this._gridHelper.AddColumn("Ts_ItemCode",			"产品",null);
			this._gridHelper.AddColumn("Ts_MoCode",			"工单",null);
			this._gridHelper.AddColumn("Ts_NGDate",				"不良日期",null);
			this._gridHelper.AddColumn("Ts_NGTime",			"不良时间",null);
			this._gridHelper.AddColumn("Ts_SourceResource",		"来源站",null);	
			this._gridHelper.AddColumn("Ts_FrmUser",		"操作工",null);
            this._gridHelper.AddColumn("Ts_FrmMemo",        "送修备注", null);	
			this._gridHelper.AddColumn( "Ts_ConfirmDate",			"接收日期",	null);
			this._gridHelper.AddColumn( "Ts_ConfirmTime",		"接收时间",	null);
			this._gridHelper.AddColumn( "Ts_ConfirmResource",		"接收站",	null);	
			this._gridHelper.AddColumn( "Ts_ConfirmUser",		"接收人",null);
			this._gridHelper.AddColumn( "Ts_SouceResourceDate",			"维修日期",	null);
			this._gridHelper.AddColumn( "Ts_SouceResourceTime",		"维修时间",	null);
			this._gridHelper.AddColumn( "Ts_RepaireResource",		"维修站",	null);	
			this._gridHelper.AddColumn("Ts_TSUser",		"维修工",null);	
			this._gridHelper.AddColumn("Ts_DestOpCode",		"去向工序",null);	
			this._gridHelper.AddColumn("Ts_ScrapCause",		"包含原因",null);
		...
		private void QueryEvent(object sender, EventArgs e)
		{
			...
			{	
                ...
                int TSEndTime = 0; //FormatHelper.TOTimeInt(this.txtTSEndTime.Text);
