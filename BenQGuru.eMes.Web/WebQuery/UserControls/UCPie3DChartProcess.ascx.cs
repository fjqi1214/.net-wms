...
{
    public partial class UCPie3DChartProcess : System.Web.UI.UserControl
    {
        ...
        private void DefineChartStyle()
        {
            ...
            {
                this.PieChart3D.Axis.X.Extent = DataSource[DataSource.Length - 1].PeriodCode.Length;
                this.PieChart3D.Axis.Y.Labels.ItemFormatString = YLabelFormatString;
                this.PieChart3D.PieChart3D.ChartText[0].ItemFormatString = ChartTextFormatString;
                this.PieChart3D.ChartType = Infragistics.UltraChart.Shared.Styles.ChartType.PieChart3D ;
                if (m_Width != 0)
                {
                    PieChart3D.Width = m_Width;
                }

                if (m_Height != 0)
                {
                    PieChart3D.Height = m_Height;
                }
            }
        }
            ...
            foreach (NewReportDomainObject domainObject in DataSource)
            {
                ...
                if (groupColumns.Length > 0)
                {
                    for (int i = 0; i < groupColumns.Length; i++)
                    {
                        ...
                        else if (groupColumns[i].Trim().ToLower() == "summaryTarget")
                        {
                            name += domainObject.DutyCode + "-";
                        }
                    }
