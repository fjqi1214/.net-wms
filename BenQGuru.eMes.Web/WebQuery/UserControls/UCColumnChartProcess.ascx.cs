...
namespace BenQGuru.eMES.Web.WebQuery.UserControls
{
    public partial class UCColumnChartProcess : System.Web.UI.UserControl
    {
        ...
        public void DataBindTable()
        {
            ...
            this.columnChart.Data.SwapRowsAndColumns = true;
            this.columnChart.DataSource = ColumnDataSource;
            this.columnChart.DataBind();            
        }

        public void DataBindQty(string Name)
        {
            DefineChartStyle();

            DataTable data = ConvertDataTableNewQty(Name);
            columnChart.DataSource = data;
            columnChart.DataBind();
        }
            ...
            foreach (NewReportDomainObject domainObject in DataSource)
            {
                ...
                if (groupColumns.Length > 0)
                {
                    for (int i = 0; i < groupColumns.Length; i++)
                    {
                        ...
                        else if (groupColumns[i].Trim().ToLower() == "customercode")
                        {
                            name += domainObject.CustomerCode + "-";
                        }
                        else if (groupColumns[i].Trim().ToLower() == "faccode")
                        {
                            name += domainObject.FacCode + "-";
