...
{
	/// <summary>
	/// FReportCenterMonthProduct 的摘要说明。
	/// </summary>
	public partial class FReportCenterMonthProduct : BaseRQPage
	{
		...
		{
			this._gridHelper = new GridHelperForRPT(this.gridMonthProduct);
		...
		{
			this.gridMonthProduct.Columns.Clear();
			...
			if( this.gridMonthProduct.Columns.FromKey( selected ) != null )
			{
                this.gridMonthProduct.Columns.FromKey(selected).HeaderStyle.BackColor = Color.SkyBlue;
			}
		}

		private object[] _loadDataSource()
		{
			return new FacadeFactory(base.DataProvider).CreateQueryFacade1().QueryRPTCenterMonthProduct(today,opCode,eCode);
		}
		...
		private void _processDataDourceToGrid(object[] source)
		{
			...
			this.gridMonthProduct.Rows.Clear();

			if( source != null )
			{
				foreach(RPTCenterMonthProduct real in source)
				{
					UltraGridRow gridRow = null;
					object[] objs = new object[this.gridMonthProduct.Columns.Count];
					gridRow = new UltraGridRow( objs );
					this.gridMonthProduct.Rows.Add( gridRow );
