...
{
	/// <summary>
	/// ReportCenterMonthQuantity 的摘要说明。
	/// </summary>
	public partial class ReportCenterMonthQuantity : BaseRQPage
	{
		...
		{
			return new FacadeFactory(base.DataProvider).CreateQueryFacade1().QueryRPTCenterMonthQuantity(today,segmentCode);
		}
		...
		private void _processOWC(object[] dataSource)
		{
			...
			if( dataSource != null )
			{
				...
				{
					categories[i] = (dataSource[i] as RPTCenterMonthQuantity).ShiftDay.ToString();
					values[i] = (dataSource[i] as RPTCenterMonthQuantity).DayQuantity;
				}
