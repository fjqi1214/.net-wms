...
namespace BenQGuru.eMES.Web.WebQuery
{
    ...
    /// </summary>
    public partial class FRealTimeInputOutputQTY : BaseQPageNew
    {
        ...
        private void _initialWebGrid()
        {
            ...
            this.gridHelper.AddColumn("MOPlanqty", "工单计划数", null);
            this.gridHelper.AddColumn("MOShiftInputqty", "工单投入数(当班)", null);
            this.gridHelper.AddColumn("MOInputqty", "工单投入数(累计)", null);
            this.gridHelper.AddColumn("MOShiftOutputqty", "工单产出数(当班)", null);
            this.gridHelper.AddColumn("MOOutputqty", "工单产出数(累计)", null);
            this.gridHelper.AddColumn("MOScrapqty", "工单拆解数(累计)", null);
            this.gridHelper.AddColumn("MOOffQty", "工单脱离数(累计)", null);
        ...
        private void _processDataDourceToGrid(object[] source)
        {
            ...
            if (source != null)
            {
                foreach (RealTimeInputOutputQuantity real in source)
                {
                    ...
                    this.DtSource.Rows.Add(this.GetGridRow(real));
                }
            }
        ...
        private string GetUrlParams(object obj)
        {
            ...
            string returnStr = string.Format("{0}?post='true'&segmentcode={1}&shiftday={2}&shiftcode={3}&modelcode={4}&Itemcode={5}&mocode={6}",
                                                    "FRealTimeQuantitySummaryQP.aspx",
                                                    this.drpSegmentQuery.SelectedValue,
                                                    this.eMESDate1.Text,
                                                    this.drpShiftQuery.SelectedValue,
                                                    rObj.ModelCode,
