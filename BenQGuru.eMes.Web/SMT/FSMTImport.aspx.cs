...
namespace BenQGuru.eMES.Web.SMT
{
	...
	/// </summary>
	public class FSMTImport  : BaseMPage
	{
		...
		protected System.Web.UI.HtmlControls.HtmlInputButton cmdPending;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			//SyncMOCode();	//同步工单,针对用户改变txtMOCode的值但是不选择工单
		...
		//同步工单
		private void SyncMOCode()
		{
			if(this.hidtxtMOCode.Value != string.Empty && this.hidtxtMOCode.Value != this.txtMOCode.Text)
			{
				this.txtMOCode.Text = this.hidtxtMOCode.Value;
			}
		}
		#endregion
		...
		protected override object[] LoadDataSource(int inclusive, int exclusive)
		{
			...
			{
				//通过当前维护工单和机台查询SMTResourceBOM this.txtMOCode.Text
				//return this._facade.QuerySMTResourceBOM(string.Empty , this.drpMOCode.SelectedValue , string.Empty, string.Empty, this.treeWebTree.SelectedNode.Text , string.Empty,inclusive,exclusive) ;
				return this._facade.QuerySMTResourceBOM(string.Empty , this.txtMOCode.Text , string.Empty, string.Empty, this.treeWebTree.SelectedNode.Text , string.Empty,inclusive,exclusive) ;
			}
			else
			{
				return null ;
...
//			}
		...
		{
			if(_facade==null){_facade = new SMTFacadeFactory(base.DataProvider).Create();}
			//if( this.drpMOCode.SelectedValue != string.Empty )this.txtMOCode.Text
			if( this.txtMOCode.Text != string.Empty )
			{
				//return this._facade.QuerySMTResourceBOMCount(string.Empty , this.drpMOCode.SelectedValue , string.Empty, string.Empty, this.treeWebTree.SelectedNode.Text , string.Empty) ;
				return this._facade.QuerySMTResourceBOMCount(string.Empty , this.txtMOCode.Text , string.Empty, string.Empty, this.treeWebTree.SelectedNode.Text , string.Empty) ;
			}
			else
			{
				return 0 ;
		...
		protected override object GetEditObject()
		{
			...
			bom.MOCode = FormatHelper.PKCapitalFormat(FormatHelper.CleanString(this.txtMOCode.Text)) ;
...
//				FormatHelper.PKCapitalFormat(FormatHelper.CleanString(row.Cells.FromKey("MaterialItemCode").Text)));
		...
		protected override string[] FormatExportRecord( object obj )
		{
			...
			else
			{
				//this.drpMOCode.SelectedValue,
				//导出数据 没有Feeder栏位
				return new string[]{  this.txtMOCode.Text,
									   ((SMTResourceBOM)obj).ResourceCode.ToString(),
									   ((SMTResourceBOM)obj).StationCode.ToString(),
									   ((SMTResourceBOM)obj).OPBOMItemCode.ToString()
								   };
...
//			}
			if(this.txtMOCode.Text != "")
			{
				this.drpMOCopySourceQuery.Items.Remove(this.txtMOCode.Text);	//移除当前的工单(参数为当前的工单Code)
			}
		...
		private void drpMOCode_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			//设置产品显示
			MO currentMO = (new SMTFacadeFactory(base.DataProvider).CreateMOFacade().GetMO(this.txtMOCode.Text) as MO);
			//MO currentMO = (SMTFacadeFactory.CreateMOFacade().GetMO(this.drpMOCode.SelectedValue) as MO);
			if(currentMO!=null)
			{
		...
		private void BuildRouteTree()
		{
			...
			//string moCode = this.drpMOCode.SelectedValue ;
		...
		private void cmdCopy_ServerClick(object sender, System.EventArgs e)
		{
			...
			Hashtable returnResCodeHT = _facade.CheckSameResource(this.txtMOCode.Text,selectResCodeList);
			...
			if(sameResCodeList.Count >0 )
			{
				...
				sessionHT["ToMOCode"] = this.txtMOCode.Text;
			...
			else
			{
				...
				if(diffResCodeList.Count!=0)
				{
					...
					_facade.CopySMTResourceBOM(this.drpMOCopySourceQuery.SelectedValue,this.txtMOCode.Text,diffResCodeList);
					BuildRouteTree();
					this.Alert("复制完成");
				}
			}
		...
		private void cmdCompare_ServerClick(object sender, System.EventArgs e)
		{
			...
			if(this.rblMOBOMSourceSelect.SelectedValue == CompareSourceType.DB)
			{
				...
				object[] dbmoboms = mofacade.GetMOBOM(this.hidtxtMOCode.Value,this.txtSapOPCode.Text); //根据工单和OPCode获取MOBOM
		...
		private object[] GetImportStationItemBOM()
		{
			...
			//object[] filterboms = _facade.FilterImportData(this.drpMOCode.SelectedValue,smtboms);	//比对检查,返回可以导入的数据
			object[] filterboms = _facade.FilterImportData(this.txtMOCode.Text,smtboms);	//比对检查,返回可以导入的数据

			foreach(SMTResourceBOM SMTrBOM in filterboms)
			{
				//映射数据以及数据格式
				//SMTrBOM.MOCode = FormatHelper.PKCapitalFormat(this.drpMOCode.SelectedValue);
				SMTrBOM.MOCode = FormatHelper.PKCapitalFormat(this.txtMOCode.Text);
		...
		protected object[] LoadExportData()
		{
			...
			object[] boms = this._facade.QuerySMTResourceBOMExport(this.txtMOCode.Text, ResourceCodeList) ;
	...
	public class CompareSourceType : IInternalSystemVariable
	{
		private ArrayList _list = new ArrayList();

		public CompareSourceType()
		{
			this._list.Add(CompareSourceType.DB);
			this._list.Add(CompareSourceType.Excel);
		}

		public  const string DB ="comparesourcetype_db";
		public  const string Excel ="comparesourcetype_excel";

		public static string[] ParserAttributeTimingType(string comparesourceType)
		{
			if( comparesourceType.ToUpper() == CompareSourceType.DB.ToUpper() )
			{
				return new string[]{"comparesourcetype_db","CompareSourceType"};
			}
			else if( comparesourceType.ToUpper() == CompareSourceType.Excel.ToUpper() )
			{
				return new string[]{"comparesourcetype_excel","CompareSourceType"};
			}
